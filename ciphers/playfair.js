class PlayFair {
  ...
  preProcess(input) {
    ...
    for (let i = 0; i < text.length; i += 2) {
      ...
      if (currentDuple.length !== 2) {
        currentDuple.push("x");
        duples.push(currentDuple);
      } else if (currentDuple[0] === currentDuple[1]) {
        text.splice(i + 1, 0, "x");
  ...
  encrypt(input) {
    ...
    // if (input && input.length % 2 !== 0) return "Invalid ciphertext";
    const coordinates = this.preProcess(input);
    ...
    const processedText = processedLocs
      .map((loc) =>
        [this.grid[loc[0][0]][loc[0][1]], this.grid[loc[1][0]][loc[1][1]]].join(
          ""
        )
      )
      .join("");
